...
uint8_t ssd1306_Init(void)
{
  ...
  ssd1306_WriteCommand(0x95); //0x14
  ssd1306_WriteCommand(MEMORYMODE);
  ssd1306_WriteCommand(0x00);
  ssd1306_WriteCommand(SEGREMAP);
  ssd1306_WriteCommand(0xC8); //ssd1306_WriteCommand(COMSCANINC);
  ...
  /*
	...
	// 1. Sprawdź obecność
	  if (HAL_I2C_IsDeviceReady(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 5, 100) != HAL_OK) {
	    SSD1306.Initialized = 0;
	    return 0;
	  }

	  // 2. Twarda inicjalizacja dla 128x32 (bez if-ów)
  uint8_t init_cmds[] = {
      0x00,       // Control Byte
      0xAE,       // Display OFF
      0xD5, 0x80, // Clock Divide (domyślne)
      0xA8, 0x1F, // Multiplex Ratio = 31 (czyli 32 linie - KLUCZOWE!)
      0xD3, 0x00, // Offset = 0
      0x40,       // Start Line = 0
      0x8D, 0x95, // Charge Pump = ON
      0x20, 0x00, // Memory Mode = Horizontal
      0xA1,       // Segment Remap = 127->0 (Flip X) - ZMIEŃ NA 0xA0 JEŚLI OBRAZ LUSTRZANY
      0xC8,       // COM Scan = N-1->0 (Flip Y)      - ZMIEŃ NA 0xC0 JEŚLI OBRAZ DO GÓRY NOGAMI
      0xDA, 0x02, // COM Pins = Sequential (Dla 128x32 - KLUCZOWE!)
      0x81, 0x8F, // Contrast
	  ...
	  // Wyślij wszystko jedną paczką (szybciej i pewniej)
	  HAL_I2C_Master_Transmit(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, init_cmds, sizeof(init_cmds), 100);

	  // 3. Wyczyść i wyświetl
	  SSD1306.CurrentX = 0;
	  SSD1306.CurrentY = 0;
	  SSD1306.Color = Black;
	  SSD1306.Initialized = 1;
...
//  Position the cursor
//
void ssd1306_SetCursor(int16_t y, int16_t x)
{
  SSD1306.CurrentX = x;
  SSD1306.CurrentY = y;
}
